...
fn test_all_supported_modules_have_parameter_handlers() {
    ...
    // Report findings
    println!("Modules with working parameter handlers: {working_handlers:?}");
    ...
    assert!(
        ...
        "The following supported modules are missing parameter handlers: {missing_handlers:?}. \
...
fn test_essential_modules_are_registered() {
    ...
    assert!(
        ...
        "Essential modules missing parameter handlers: {missing_essential:?}"
    );
}
...
fn test_parameter_handler_consistency() {
    ...
    for (module_name, required_params, _optional_params) in test_cases {
        ...
        if !required_params.is_empty() {
            assert!(
                empty_result.is_err(),
                "Module '{module_name}' should fail with empty parameters since it has required params: {required_params:?}"
            );
        }
        ...
        assert!(
            ...
            "Module '{module_name}' should succeed with required parameters, but got: {full_result:?}"
        );
    }
}
    ...
    for (module_name, params) in edge_cases {
        ...
        match result {
            ...
            Err(other) => {
                ...
                println!("Edge case '{module_name}' failed with: {other:?}");
            }
        }
    }
}
