use rustle_deploy::compilation::output::{BinaryOutputManager, CopyResult};
use rustle_deploy::compilation::{BinarySource, CompiledBinary, CompilationCache, OptimizationLevel};
use chrono::Utc;
...
fn create_test_binary_with_project() -> (CompiledBinary, TempDir) {
    let temp_dir = TempDir::new().unwrap();
    let project_path = temp_dir.path().join("test-binary");
    
    // Create a fake binary file in project
    std::fs::write(&project_path, b"fake binary data").unwrap();
    
    let binary = CompiledBinary {
        binary_id: "test-binary-id".to_string(),
        target_triple: "x86_64-unknown-linux-gnu".to_string(),
        binary_path: project_path.clone(),
        binary_data: b"fake binary data".to_vec(),
        effective_source: BinarySource::FreshCompilation { project_path },
        size: 16,
        checksum: "test-checksum".to_string(),
        compilation_time: std::time::Duration::from_secs(1),
        optimization_level: OptimizationLevel::Release,
...
#[tokio::test]
async fn test_project_binary_copy() {
    let manager = BinaryOutputManager::new(create_test_cache());
    let (binary, _temp_source) = create_test_binary_with_project();
